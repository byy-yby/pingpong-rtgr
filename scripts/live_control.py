...
class LiveControl:
    ...
    # 组顺序即身份 ID（0/1）；换机位/换边时改这里。换广角镜头后每相机可能同时框到
    # 近/远两人，匹配前用 keep_nearest_person 每相机只留最近（bbox 最大）的那一个，
    # 把远处/对面的人排除，恢复「每组相机各看一人」的前提。
    ...
    def _reconstruct_frame(self) -> None:
        """批处理检测所有相机 + 排除远处人 + 固定分组匹配 + 三角化，更新 Open3D 骨架与 2D 姿态。"""
        from tabletennis.reconstruction import keep_nearest_person, match_people_fixed
        ...
        # 排除远处/对面的人：每相机只留 bbox 最大（最近）的那一个，让固定分组匹配稳定
        poses_per_cam = keep_nearest_person(poses_per_cam)
